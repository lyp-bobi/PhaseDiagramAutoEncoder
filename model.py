...
class LatentAttention():
    def __init__(self):
        self.dataSource = input_data.read_data_sets("", one_hot=True)
        self.n_samples = self.dataSource.train.num_examples
        ...
        self.generation_loss = -tf.reduce_sum(self.images * tf.log(1e-5 + generated_flat) + (1-self.images) * tf.log(1e-5 + 1 - generated_flat),1)
    ...
    def train(self):
        visualization = self.dataSource.validation.next_batch(self.batchsize)
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(5000):
                for idx in range(int(self.n_samples / self.batchsize)):
                    batch = self.dataSource.train.next_batch(self.batchsize)
    ...
    def encode(self,img):
        ...
        with tf.Session() as sess:
            ...
            res=sess.run(self.feature,feed_dict={self.images:img})
            return res
